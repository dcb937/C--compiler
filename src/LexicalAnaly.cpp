...
void LexicalAnaly::performLexicalAnalysis()
{
    ...
    ifstream input("testcases/10.txt"); // 打开输入文件
    ofstream output("output/lex.txt");  // 打开输出文件
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            if (isalpha(line[i]) || line[i] == '_') { // 处理标识符
                ...
                if (keywords.count(idn)) { // 如果是关键字
                    currentState = DFA[currentState].transitions[7];
                ...
                if (isNegative && !isdigit(line[i + 1])) { // 如果是减号运算符
                                                           // ch=2;
                    currentState = 0;
                    currentState = DFA[0].transitions[7];
                    currentState = DFA[7].transitions[2];
                    cout << type[currentState] << endl;
...
void LexicalAnaly::initializeDFA()
{
    ...
    DFA[7].transitions[2] = 2;
    DFA[7].transitions[3] = 3;
    DFA[7].transitions[4] = 4;
    DFA[0].transitions[5] = 5;
    DFA[0].transitions[6] = 6;
    DFA[0].transitions[7] = 7;
    //初始状态
